...
struct Opt {
    ...
    #[arg(long, env = "WEBDAV_ADMIN_HOST", default_value = "0.0.0.0")]
    admin_host: String,
    /// Admin API listen port
    #[arg(long, env = "WEBDAV_ADMIN_PORT", default_value = "12345")]
    admin_port: u16,
}
...
async fn cache_invalidate_handler(
...
) -> Result<Json<CacheInvalidateResponse>, (StatusCode, Json<ErrorResponse>)> {
    ...
    cache.invalidate_path_and_parent(&path).await;
...
fn start_admin_server(cache: Arc<Cache>, host: String, port: u16) {
    tokio::spawn(async move {
        let app = Router::new()
        ...
        let listener = match tokio::net::TcpListener::bind(&addr).await {
            ...
            Err(err) => {
                error!(error = %err, "admin server bind failed");
                return;
            }
        };
...
async fn main() -> anyhow::Result<()> {
    ...
    start_admin_server(cache.clone(), opt.admin_host.clone(), opt.admin_port);
    #[cfg(unix)]
